...
const RegisterPage = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      pushToast(t("auth.account_created"), t("auth.welcome_user", { username: response.user.username }));
      navigate("/app/home");
    } catch (error) {
    ...
    <main className="relative grid h-full place-items-center bg-paw-bg-primary px-4">
      ...
      >
        ...
        <p className="mt-5 text-sm text-paw-text-secondary">
          {t("auth.already_have_account")} <Link to="/login" className="text-paw-accent hover:underline">{t("auth.login_link")}</Link>
        </p>
      </form>
    </main>
  );
